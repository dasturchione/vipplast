@foreach ($data as $category)
    <li class="dd-item" data-id="{{ $category->id }}">
        <div class="dd-handle" style="position:relative">
            <div>{{ $category->name_uz }}</div>
        </div>
        <div class="flex space-x-3 btn-icons" style="position:absolute;top:5px;right:10px;">
        ...
        @if ($category->sub->count())
            <ol class="dd-list">
                @include('admin.pages.category.nestable', ['data' => $category->sub])
            </ol>
        @endif
    </li>
@endforeach
