            ...
            <div
                ...
                <div class="flex flex-col gap-4 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <button onclick="window.location.href='{{ route('admin.banners.form', ['action' => 'create']) }}'"
                            ...
                            @foreach ($data as $banner)
                                <tr class="border-b border-gray-100 dark:border-gray-800">
                                    <td class="px-5 py-4 sm:px-6 w-95" colspan="1">
                                        <div x-data="imageInfo('{{ asset('storage/' . $banner->image) }}')" x-init="init()"
                                            class="flex items-center gap-3">
                                            <div class=" h-20 overflow-hidden rounded-[5px]">
                                                <img class="h-20" :src="url" @load="onLoad($event)"
                                                    alt="{{ $banner->title_uz }}" />
                                            </div>
                                            <div>
                                                <span
                                                    class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                                    O'lchami: <span x-text="width + ' x ' + height + ' px'"></span>
                                                </span>
                                                <span class="block text-gray-500 text-theme-xs dark:text-gray-400">
                                                    Bannerga mosligi:
                                                    <span x-text="isBannerCompatible() ? '✅ Mos' : '❌ Mos emas'"></span>
                                                </span>
                                            </div>
                                        </div>
                                    </td>
        ...
        : 'bg-error-50 text-error-700 dark:bg-error-500/15 dark:text-error-500' }}">
                                    ...
                                    <td class="px-5 py-4 sm:px-6 ">
                                        <div class="flex items-center space-x-2">
                                            <a
                                                href="{{ route('admin.banners.form', ['action' => 'edit', 'id' => $banner->id]) }}">
    ...
    <script>
        function imageInfo(imageUrl) {
            return {
                ...
                height: 0,
                init() {
                    // Yuklangandan keyin rasmni avtomatik o'qiydi
                    const img = new Image();
                    img.onload = () => {
                ...
                onLoad(e) {
                    // Agar siz @load ishlatmoqchi bo‘lsangiz
                    this.width = e.target.naturalWidth;
                    this.height = e.target.naturalHeight;
                },
                isBannerCompatible() {
                    return this.width === 1905 && this.height === 900;
                }
            }
        }
    </script>
